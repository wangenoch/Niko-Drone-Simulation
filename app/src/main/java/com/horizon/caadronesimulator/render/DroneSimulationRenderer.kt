...
class DroneSimulationRenderer(private val onFlightDataUpdate: (Float, Float, Float, Float, Float, Float, Float, Boolean) -> Unit) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        zoomPipRect?.let { rect ->
            ...
            // 觀察點固定在 (0, observerHeight, -22), 目標在 (curX, curY, curZ)
            val distToCam = sqrt(curX.pow(2) + (observerHeight - curY).pow(2) + (curZ + 22f).pow(2))
    ...
    private fun applyWindForces(dt: Float, mass: Float) {
        ...
        // 氣壓與垂直亂流模擬 (優化：偶發性間歇發生)
        if (enableAirPressure && windLevel > 0) {
            turbulencePhase += dt * (0.5f + windVariation * 0.3f)
            
            // 使用一個超低頻的正弦波 (約 10-15 秒一個週期) 作為「氣流活動窗口」
            // 當 probability > 0.4 時才會有明顯的垂直位移
            val probability = sin(turbulencePhase * 0.4f) * 0.5f + 0.5f // 映射到 0.0 ~ 1.0
            
            if (probability > 0.6f) {
                // 在活動窗口內，產生一個偶發的上升或下降脈衝
                val vWave = sin(turbulencePhase * 2.5f) * 0.9f + sin(turbulencePhase * 5.0f) * 0.1f
                // 強度受機率影響：越接近窗口中心強度越強，形成「突發 -> 漸弱」的效果
                val burstIntensity = (probability - 0.6f) * 2.5f 
                val vForce = vWave * (windLevel * 0.8f) * (1.0f + windVariation * 0.6f) * burstIntensity / mass
                velY += vForce * dt
            }
        }
    ...
    private fun updateCameraMatrix(mode: String) {
        when (mode) {
            "站位視角 (追蹤)" -> Matrix.setLookAtM(vMatrix, 0, 0f, observerHeight, -22f, curX, curY, curZ, 0f, 1f, 0f)
            "站位視角 (固定)" -> Matrix.setLookAtM(vMatrix, 0, 0f, observerHeight, -22f, 0f, 0f, 0f, 0f, 1f, 0f)
            "跟隨視角" -> {
            ...
            "姿態輔助視角" -> {
                ...
                when {
                    cameraMode.contains("站位視角") -> {
                        // 從觀察者原位發射的望遠鏡視角
                        Matrix.setLookAtM(vMatrix, 0, 0f, observerHeight, -22f, curX, curY, curZ, 0f, 1f, 0f)
                    }
                    cameraMode == "跟隨視角" -> {
                        // 從跟隨攝影機位置放大的視角
                        val rad = Math.toRadians(curYaw.toDouble()).toFloat()
    ...
    fun resetFlight() { 
        curX = 0f; curY = getGroundY(); curZ = hZ; curYaw = 0f; velX = 0f; velY = 0f; velZ = 0f; visPitch = 0f; visRoll = 0f; zoomFactor = 1.5f; observerHeight = 6.0f
