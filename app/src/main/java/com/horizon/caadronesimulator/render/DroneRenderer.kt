...
object DroneRenderer {
    ...
    private fun drawHeavyLiftDrone(posH: Int, colorH: Int, mvpH: Int, mvpMatrix: FloatArray, x: Float, y: Float, z: Float, yaw: Float, pitch: Float, roll: Float, time: Float, isMotorLocked: Boolean) {
        ...
        arrayOf(legX, -legX).forEach { lx ->
            // 將底部橫桿長度從 1.1s 縮短至 0.9s (前後向)
            RenderUtils.drawBox(posH, colorH, mvpH, mvpMatrix, baseM, lx, -0.45f*s, 0f, 0.04f*s, 0.04f*s, 0.9f*s, floatArrayOf(0.1f, 0.1f, 0.1f, 1f))
            // 同步移動兩端裝飾件的位置從 0.6s 移至 0.5s
            arrayOf(0.5f*s, -0.5f*s).forEach { tz -> RenderUtils.drawBox(posH, colorH, mvpH, mvpMatrix, baseM, lx, -0.45f*s, tz, 0.05f*s, 0.05f*s, 0.15f*s, if(lx>0) floatArrayOf(1f,0f,0f,1f) else floatArrayOf(0f,1f,0f,1f)) }
        }
    }
