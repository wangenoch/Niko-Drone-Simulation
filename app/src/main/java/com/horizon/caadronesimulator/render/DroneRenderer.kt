...
object DroneRenderer {
    ...
    fun drawActiveDrone(posH: Int, colorH: Int, mvpH: Int, mvpMatrix: FloatArray, droneType: String, curX: Float, curY: Float, curZ: Float, curYaw: Float, visPitch: Float, visRoll: Float, flightTime: Float, isMotorLocked: Boolean, motorRpmFactor: Float = 1.0f) {
        val spec = DroneRegistry.getSpec(droneType)
        val drawY = curY + spec.visualOffset 
        when (droneType) {
            "HEAVY_LIFT" -> drawHeavyLiftDrone(posH, colorH, mvpH, mvpMatrix, curX, drawY, curZ, curYaw, visPitch, visRoll, flightTime, isMotorLocked, motorRpmFactor)
            "HELI_900" -> drawHelicopter(posH, colorH, mvpH, mvpMatrix, curX, drawY, curZ, curYaw, visPitch, visRoll, flightTime, isMotorLocked, motorRpmFactor)
            else -> drawDrone(posH, colorH, mvpH, mvpMatrix, curX, drawY, curZ, curYaw, visPitch, visRoll, flightTime, isMotorLocked, motorRpmFactor)
        }
    }

    private fun drawHelicopter(posH: Int, colorH: Int, mvpH: Int, mvpMatrix: FloatArray, x: Float, y: Float, z: Float, yaw: Float, pitch: Float, roll: Float, time: Float, isMotorLocked: Boolean, motorRpmFactor: Float) {
        ...
        val mainPropSpd = if(!isMotorLocked) time*2200f*motorRpmFactor else 0f
        ...
        val tailPropSpd = if(!isMotorLocked) time*4000f*motorRpmFactor else 0f
    ...
    private fun drawHeavyLiftDrone(posH: Int, colorH: Int, mvpH: Int, mvpMatrix: FloatArray, x: Float, y: Float, z: Float, yaw: Float, pitch: Float, roll: Float, time: Float, isMotorLocked: Boolean, motorRpmFactor: Float) {
        ...
        arrayOf(floatArrayOf(1f,1f), floatArrayOf(-1f,1f), floatArrayOf(1f,-1f), floatArrayOf(-1f,-1f)).forEach { p ->
            ...
            RenderUtils.drawBox(posH, colorH, mvpH, mvpMatrix, baseM, p[0]*armLen*0.707f, 0.2f, p[1]*armLen*0.707f, 0.7f*s, 0.01f, 0.04f*s, floatArrayOf(0.9f, 0.9f, 0.9f, 0.5f), if(!isMotorLocked) time*2500f*(p[0]*p[1])*motorRpmFactor else 0f)
        }
    ...
    private fun drawDrone(posH: Int, colorH: Int, mvpH: Int, mvpMatrix: FloatArray, x: Float, y: Float, z: Float, yaw: Float, pitch: Float, roll: Float, time: Float, isMotorLocked: Boolean, motorRpmFactor: Float) {
        ...
        val pSpd = if(!isMotorLocked) time*2000f*motorRpmFactor else 0f
