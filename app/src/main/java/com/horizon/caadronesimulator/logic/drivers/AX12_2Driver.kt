...
/**
 * [v1.5.2] RadioMaster AX-Enhanced (AX12-2) 驅動程式
 * 針對新版 RadioMasterAX 硬體環境優化：
 * 1. 提升波特率至 460800。
 * 2. 實作滑動視窗同步與容錯機制。
 * 3. 支援 0x55 0xAA 軟啟動序列。
 */
class AX12_2Driver : InternalDeviceDriver {
    override val brandName = "RadioMaster AX-Enhanced"
    override var defaultBaudRate = 460800 // [v1.5.2] 改為 var 允許動態覆蓋
    ...
    override val recommendedBaudRate = 460800
    override val protectionWarning = "RadioMaster AX-Enhanced 專業協議已鎖定。"
    ...
    override fun parseRaw(buffer: ByteBuffer): List<Float> {
        // [v1.5.2] 針對 Enhanced 版本修正物理偏移與對齊
        // 假設新版協議將 Throttle 置於首位
        val throttle = (buffer.getShort(6).toFloat() / 500.0f).coerceIn(-1f, 1f)
        val yaw = (buffer.getShort(8).toFloat() / 500.0f).coerceIn(-1f, 1f)
        val pitch = (buffer.getShort(10).toFloat() / 500.0f).coerceIn(-1f, 1f)
        val roll = (buffer.getShort(12).toFloat() / 500.0f).coerceIn(-1f, 1f)

        val pool = mutableListOf<Float>()
        pool.add(throttle) // 101
        pool.add(yaw)      // 102
        pool.add(pitch)    // 103
        pool.add(roll)     // 104
        ...
        for (i in 0 until 20) {
            ...
            if (offset + 2 <= buffer.capacity()) {
                ...
                pool.add((rawVal / 500.0f).coerceIn(-1f, 1f))
            } else pool.add(0f)
        }
        return pool
    }
