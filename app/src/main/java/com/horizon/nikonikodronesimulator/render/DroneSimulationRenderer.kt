...
import com.horizon.nikonikodronesimulator.render.util.FboManager
...
class DroneSimulationRenderer(private val onFlightDataUpdate: (Float, Float, Float, Float, Float, Float, Float, Boolean, Float, Int, androidx.compose.ui.geometry.Offset?, Float?, Float?, Float?, Float?) -> Unit) : GLSurfaceView.Renderer {
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        if (currentTitleText.isBlank()) {
            ...
            currentTitleText = com.horizon.nikonikodronesimulator.model.AppConfig.getDefaultSpecialTitle(ds.appLanguage)
        }
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        val ds = com.horizon.nikonikodronesimulator.model.DroneState.getInstance()
        com.horizon.nikonikodronesimulator.logic.WindManager.update(this.rendererTime, windLevel, windVariation.toInt(), useHardcorePhysics)
        
        // [v1.7.7 核心修正] 傳遞 rendererTime 替代 flightTime，強制亂數風向與渲染幀同步
        // [v1.7.9 唯一寫回點] 由 Renderer 接管數據更新主權
        val windResult = com.horizon.nikonikodronesimulator.logic.WindManager.calculateWindVector(
        ...
        if (!isPaused) {
            ...
            val atmos = com.horizon.nikonikodronesimulator.logic.PhysicsEngine.AtmosConfig(
                windLevel, windDirection, windVariation.toInt(), windDirVariation.toInt(),
            ...
            val result = com.horizon.nikonikodronesimulator.logic.PhysicsEngine.step(
                dt, physicsState, 
                com.horizon.nikonikodronesimulator.logic.PhysicsEngine.ControlInput(ctrlThrottle, ctrlYaw, ctrlPitch, ctrlRoll),
                atmos, droneType
            )
            
            com.horizon.nikonikodronesimulator.logic.CameraDirector.update(
                ...
                lastManualTouchTime, droneType, dt, com.horizon.nikonikodronesimulator.model.DroneState.getInstance()
            )
        ...
        zoomPipRect?.let {
            ...
            GLES20.glClearColor(skyColor[0], skyColor[1], skyColor[2], skyColor[3])
            GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT or GLES20.GL_DEPTH_BUFFER_BIT)
            Matrix.perspectiveM(pMatrix, 0, com.horizon.nikonikodronesimulator.logic.CameraDirector.smoothedZoomPipFov, 1f, 0.1f, 1000f)
            com.horizon.nikonikodronesimulator.logic.CameraDirector.computePrecisionViewMatrix(vMatrix, physicsState.posX, physicsState.posY, physicsState.posZ)
            renderScene(isOffscreen = true)
            zoomFbo.unbind()
        }
        ...
        val finalFov = if (cameraMode == AppConfig.CAM_MODE_FPV) spec.fpvFov else com.horizon.nikonikodronesimulator.logic.CameraDirector.smoothedFov
    ...
    private fun renderScene(droneOnly: Boolean = false, isOffscreen: Boolean = false) {
        ...
        if (showClouds) {
            ...
            val ds = com.horizon.nikonikodronesimulator.model.DroneState.getInstance()
            val cColor = com.horizon.nikonikodronesimulator.logic.EnvironmentManager.getCloudColor(ds)
