...
object ProHardwareBridge {
    ...
    fun initialize(
        ...
        com.horizon.nikonikodronesimulator.model.AppConfig.isProVersion = true

        // [v1.7.6] 注入驅動實體，解決 src/main 找不到 src/pro 驅動程式的問題
        HardwareRegistry.driverProvider = { id ->
            when(id) {
                "RM_AX12_V1" -> com.horizon.nikonikodronesimulator.logic.drivers.AX12Driver()
                "RM_AX12_V2" -> com.horizon.nikonikodronesimulator.logic.drivers.AX12V2Driver()
                "QUALCOMM MK15" -> com.horizon.nikonikodronesimulator.logic.drivers.MK15Driver()
                else -> null
            }
        }
